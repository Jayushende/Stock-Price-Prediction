...
try:
    ...
    print("⚠ Could not fetch tickers from Wikipedia. Using fallback list.")
...
# ===============================================
def test_it(opens, closes, preds, start_account=1000, thresh=0.0):
    opens = np.asarray(opens, dtype=float).flatten()
    closes = np.asarray(closes, dtype=float).flatten()
    preds = np.asarray(preds, dtype=float).flatten()

    account = float(start_account)
    changes = []

    for i in range(len(preds)):
        if opens[i] != 0 and ((preds[i] - opens[i]) / opens[i]) >= thresh:
            account += account * (closes[i] - opens[i]) / opens[i]
        changes.append(account)

    changes = np.array(changes, dtype=float)
    plt.figure(figsize=(12, 4))
    plt.plot(changes, color='green', label='Algorithmic Trading')
    ...
    # Convert to float explicitly before rounding
    invest_total = float(start_account + start_account * (float(closes[-1]) - float(opens[0])) / float(opens[0]))
    algo_total = float(account)

    print('Simple Hold Total:', round(invest_total, 2),
          f"({round((invest_total - start_account) / start_account * 100, 1)}%)")
    print('Algo-Trading Total:', round(algo_total, 2),
          f"({round((algo_total - start_account) / start_account * 100, 1)}%)")
